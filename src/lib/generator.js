...
function buildSelector(slug, currentPageLang) {
  ...
  const links = ALL.map(lang => {
    const url = 'https://academy.ermesdance.com/' + slug + (lang === 'en' ? '' : '-' + lang);
  ...
  const initialFlag = LANG_FLAGS_HTML[currentPageLang] || LANG_FLAGS_HTML['en'];
